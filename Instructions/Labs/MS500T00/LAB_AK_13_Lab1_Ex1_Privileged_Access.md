...
ms.locfileid: "137943296"
---
# <a name="module-13---lab-1---exercise-1---set-up-privileged-access-management-and-process-a-request"></a>モジュール 13 - ラボ 1 - 演習 1 - Privileged Access Management を設定して要求を処理する
...
### <a name="task-1--create-an-approvers-group"></a>タスク 1 – 承認者のグループを作成する
...
2. **Microsoft Edge** ブラウザーで **Microsoft 365 管理センター** がタブで開いている場合は、それを選択します。開いていなければ、新しいタブを開き、アドレス バーに `https://admin.microsoft.com` の URL を入力します。

3. 管理センターで **[チームとグループ]** を選択してから **[アクティブなチームとグループ]** を選択します。  **[アクティブなチームとグループ]** 画面で **[グループの追加]** を選択します。
...
6.  グループ メール エイリアスの **[設定の編集]** 画面で `access` と入力し、次のフィールドにタブを付けて承諾可能なことを確認し、 **[次へ]** を選択します。

7.  **[確認とグループの追加の完了]** 画面で **[グループの作成]** を選択します。 **[閉じる]** を選択します。

8.  **[アクティブなチームとグループ]** 画面で、作成したばかりのグループを選択します。  グループがリストに表示されるまで数分待ってから **[更新]** をクリックする必要があるかもしれません。
...
### <a name="task-2--enable-privileged-access"></a>タスク 2 – 特権アクセスを有効にする
...
### <a name="task-3---create-a-privileged-access-policy"></a>タスク 3 - 特権アクセス ポリシーを作成する
...
### <a name="task-4---submit-a-request-for-privileged-access"></a>タスク 4 - 特権アクセスの要求を送信する
...
得られるアクセス許可:**メールボックスの移動** 

期間 (時間): `8`

コメント: `I need to move mailboxes for the new department today`。 

5. アクセス要求の処理が完了したら、 **[作成]** を選択します。 


### <a name="task-5---approve-a-privileged-access-request"></a>タスク 5 - 特権アクセス要求を承認する
...
まとめ:グローバル管理者、Holly Dickson はメールボックスを移動するために短期的な認可が必要でした。  このロールのタスクのために 8 時間のアクセス権を要求し、MOD 管理者によって承認されました。
